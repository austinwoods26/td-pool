...
export const SEASON_WEEK_LIMITS = {
  1: 4, // preseason, including the Hall of Fame Game as week 1
  2: 18, // regular season
  3: 5, // postseason, including the Pro Bowl as week 4
};
...
// Given the current (seasontype, week), return the next one,
// automatically rolling over into the next season type when needed,
// and skipping any exhibition-only weeks (like the Pro Bowl).
...
// Given the current (seasontype, week), return the previous one,
// automatically rolling back into the prior season type when needed,
// and skipping any exhibition-only weeks (like the Pro Bowl).
export function getPreviousWeek(seasontype, week) {
  ...
  if (week > 1) {
    ...
    // Already at preseason week 1 (Hall of Fame Game) — nothing before this
    return null;
  }
